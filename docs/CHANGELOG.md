...
- **RPM / Fedora packages** — `cpack -G RPM` now produces
  ...
  formats as assets.
- **Signed apt + dnf package repositories** hosted on GitHub Pages at
  <https://thecleaners.github.io/qiftop/>, generated by
  `.github/workflows/pages.yml` from the `.deb`/`.rpm` release assets
  (`dist/repo/build-pages.sh` — `apt-ftparchive` + `createrepo_c`,
  GPG-signed `Release`/`InRelease` + `repomd.xml.asc`). Package files
  stay as release assets (not in git) to keep clones lean. README
  carries one-liner `apt` / `dnf` install instructions.
