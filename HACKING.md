...
1. Bump `project(... VERSION X.Y)` in `CMakeLists.txt` (single source of
   truth — both `.deb`s and `CPACK_PACKAGE_VERSION` derive from it).
2. Update the AGENTS.md changelog.
3. Update the HACKING.md "Last reviewed" line below.
4. `git tag vX.Y && git push --tags`.
5. Build packages in a clean checkout:
...
6. Smoke-install the result on a clean container:
