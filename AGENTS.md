...
**Client controls (GUI + TUI).** `ConnectionMonitor` exposes a transport-neutral
`setDesiredAttributionEagerness(QString)` (no-op on in-process backends) plus an
`attributionEagernessChanged(QString)` signal mirrored from the agent; only the
DBus proxy forwards/emits them. The choice is a persisted preference
(`Settings::attributionEagerness`, `monitoring/attributionEagerness`, default
`""` = no hint), **re-asserted on the existing agent heartbeat** (every 4 s,
alongside the cadence hint) so it never lapses past the TTL — never on its own
timer. The GUI surfaces an "Attribution:" toolbar combo (Default / Off /
Balanced / Eager), and the TUI an `e` key that cycles the same values with a
footer hint; both are **capability-gated** on `attribution-eagerness-hints`
(hidden/ignored without it, e.g. in-process or an old agent) and reflect the
agent's effective mode (combo tooltip / flash). `default`/empty clears the
client's hint. Pinned by `test_settings_migration`
(round-trip) + `test_mainwindow_smoke::eagernessComboGatedOnCapability`.
...
| `test_settings_migration`  | `Settings` legacy-key migration logic; chip-colour + v0.2 attribution view settings (view mode, process/container column toggles, chain-in-tooltip) round-trip + out-of-range view-mode clamp; GUI theme name defaults to "System" and round-trips; runtime attribution-eagerness pref defaults empty (no hint) and round-trips (incl. clearing back to empty) |
